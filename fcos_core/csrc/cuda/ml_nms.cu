...
at::Tensor ml_nms_cuda(const at::Tensor boxes, float nms_overlap_thresh) {
  ...
  // at::cuda::getCurrentCUDAStream() *state = at::globalContext().lazyInitCUDA(); // TODO replace with getat::cuda::getCurrentCUDAStream()
  cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  ...
  // mask_dev = (unsigned long long*) c10:cuda::CUDaCachingAllocator::allocate(state, boxes_num * col_blocks * sizeof(unsigned long long));
  ...
  // c10:cuda::CUDaCachingAllocator::allocate(cudaMemcpy(&mask_host[0],
  //                       mask_dev,
  ...
  // c10:cuda::CUDaCachingAllocator::allocate(state, mask_dev);
